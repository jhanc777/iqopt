...
class IQ_Option:
    ...
    # -------------------------------------------------------------------
    #version 7 de estas funciones
    def __get_binary_open_v7(self):
        # for turbo and binary pairs
        binary_data = self.get_all_init_v2()
        binary_list = ["binary", "turbo"]
        if binary_data:
            for option in binary_list:
                if option in binary_data:
                    for actives_id in binary_data[option]["actives"]:
                        ...
                        if active["enabled"] == True:
                            if active["is_suspended"] == True:
                                self.OPEN_TIME[option][name]["suspended"] = True
                                self.OPEN_TIME[option][name]["open"] = False
                                #print(name,"esta activo, pero esta suspendido")
                            else:
                                self.OPEN_TIME[option][name]["suspended"] = False
                                self.OPEN_TIME[option][name]["open"] = True
                                #print(name,"esta activo, pero no esta suspendido")
                        else:
                            if active["is_suspended"] == True:
                                self.OPEN_TIME[option][name]["open"] = False
                                self.OPEN_TIME[option][name]["suspended"] = True
                                #print(name,"no esta activo, pero esta suspendido")
                            else:
                                self.OPEN_TIME[option][name]["open"] = False
                                self.OPEN_TIME[option][name]["suspended"] = False
                                #print(name,"no esta activo, pero no esta suspendido")

    def get_all_open_time_v7(self):
        # all pairs openned
        self.OPEN_TIME = nested_dict(3, dict)
        binary = threading.Thread(target=self.__get_binary_open_v7)
